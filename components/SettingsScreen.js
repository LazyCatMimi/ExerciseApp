...
export default function Settings({ navigation }) {
  ...
  let [userInfo, setUserInfo] = useState(initialData);
  let [cachedInfo, setCachedInfo] = useState(initialData);
    ...
    async function getUserInfo() {
      try {
        ...
        if (value !== null) {
          ...
          setCachedInfo(JSON.parse(value));
        }
      } catch (err) {
        console.error(err);
      }
  ...
  const verifyInput = (value, property) => {
    ...
    if (property === "calorieGoal" && userInfo.weight.length === 0) {
      setError({
        ...error,
        calorieGoal: "calories cannot be calculated without weight.",
      });
    }
    if (
      property === "weight" &&
      error.calorieGoal.length > 0 &&
      regex.test(value)
    ) {
  ...
  const goBack = () => {
    ...
    for (let prop in userInfo) {
      if (userInfo[prop] != cachedInfo[prop]) {
        isSame = false;
        break;
      }
    }
